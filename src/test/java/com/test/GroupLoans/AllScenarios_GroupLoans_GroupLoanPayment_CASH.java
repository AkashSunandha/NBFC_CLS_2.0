	...
	public void groupLoanPayment_CASH(Map<Object, Object> testdata, ITestContext context) throws ClassNotFoundException, InterruptedException, IOException{
		try {
			if(testdata.get("Run").toString().equalsIgnoreCase("Yes")){
				ExtentTestManager.startTest("GroupLoans_GroupLoanPayment_CASH").assignCategory("Microfinance concept-GroupLoanPayment-Cash");
				Log.info("GroupLoans_GroupLoanPayment_CASH");
				context.setAttribute("fileName", "Login");

				//TC No. - 01 --> Application launch
				ExtentTestManager.startTest("Application launch").assignCategory("Microfinance concept-GroupLoanPayment-Cash");
				Base_Class.SetUp();
