...
public class Timesheet_CancelRequestModule extends Base_Class
{
	...
	public boolean ValidateReject() throws InterruptedException 
	{
		...
		Log.info("System or PMO successfully reject the timesheet");
		return flag;
	}
...
//		FillValidHour();
	...
	public void NavigateToPreviousWeekTimeSheetWeeklyNormalView(String projectName, String moduleName, String path) throws InterruptedException
	{
...
//		select(moduleName, weekNormViewModuleSelDropdown);
        	...
        	try {
        	  ...
        	  for(int i=0;i<2;i++) {
				...
				try {
					...
					try {
						ApproveReject("Drishya Devan","Approve");
	...
	public boolean ValidateProjectAndModuleNamesDisplayed(String projectName, String moduleName) throws InterruptedException 
	{
		...
		if (flag01)
		{
			ExtentTestManager.getTest().log(Status.PASS, "Project list and module list loaded in the respective drop downs.");
			Log.info("Project list and module list loaded in the respective drop downs.");
		}
